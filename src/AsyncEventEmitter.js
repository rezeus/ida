...
class AsyncEventEmitter extends EventEmitter {
  ...
  onAsync(eventName, listener) {
    if (typeof listener !== 'function') {
      throw new TypeError(`Listener is not a function for '${eventName}' event.`);
    }
    ...
    if (onceListeners) {
      return Promise.all([
        ...listeners.map((listener) => {
          ...
          return (data.length > 0 ? listener(...data) : listener());
        }),
      ]);
    }

    return Promise.all([
      ...listeners.map((listener) => (data.length > 0 ? listener(...data) : listener())),
    ]);
  }
